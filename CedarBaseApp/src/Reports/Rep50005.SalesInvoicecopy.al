report 50005 "Sales - Invoice_copy"
{
    DefaultLayout = RDLC;
    RDLCLayout = './layout/Invoice.rdl';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(OrderDate_SalesInvHdr; "Sales Invoice Header"."Order Date")
                    {
                    }
                    ...
                    column(TotalAmountInclVAT_G; TotalAmountInclVAT_G)
                    {
                        // AutoFormatExpression = "Sales Invoice Header"."Currency Code";
                        //  AutoFormatType = 1;
                    }
                    column(TotalAmountVAT_G; TotalAmountVAT_G)
                    {
                        // AutoFormatExpression = "Sales Invoice Header"."Currency Code";
                        //  AutoFormatType = 1;
                    }
                    column(TotalSubTotal_G; TotalSubTotal_G)
                    {
                    }
                    column(TotalAmount_G; TotalAmount_G)
                    {
                    }
                    ...
                    column(InvoiceOverDueText1; InvoiceOverDueText1)
                    {
                    }
                    column(InvoiceOverDueText2; InvoiceOverDueText2)
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(InvoiceDate_SalesInvLine; "Invoice Date")
                        {
                        }
                        ...
                        begin
                            ...
                            if "Sales Invoice Line"."Carriage Line" = true then
                                Carriage := true
                            else
                                Carriage := false;
                        end;
            ...
            begin
                ...
                CarriageAmount := 0;
                SalesInvLineCarr.reset;
                SalesInvLineCarr.SetRange("Document No.", "Sales Invoice Header"."No.");
                SalesInvLineCarr.SetRange("Carriage Line", true);
                if SalesInvLineCarr.FindSet() then
                    repeat
                        CarriageAmount += SalesInvLineCarr."Line Amount";
                    until SalesInvLineCarr.Next() = 0;
                // for Total Net amount 
                TotalSubTotal_G := 0;
                SalesInvLineCarr.reset;
                SalesInvLineCarr.SetRange("Document No.", "Sales Invoice Header"."No.");
                SalesInvLineCarr.SetRange("Carriage Line", false);
                if SalesInvLineCarr.FindSet() then
                    repeat
                        TotalSubTotal_G += SalesInvLineCarr."Line Amount";
                    until SalesInvLineCarr.Next() = 0;
                // for Total Amt inc Vat
                TotalAmountInclVAT_G := 0;
                TotalAmountVAT_G := 0;
                TotalInvDiscAmount_G := 0;
                SalesInvLineCarr.reset;
                SalesInvLineCarr.SetRange("Document No.", "Sales Invoice Header"."No.");
                if SalesInvLineCarr.FindSet() then
                    repeat
                        TotalAmountVAT_G += SalesInvLineCarr."Amount Including VAT" - SalesInvLineCarr.Amount;
                        TotalAmountInclVAT_G += SalesInvLineCarr."Amount Including VAT";
                    until SalesInvLineCarr.Next() = 0;
                Clear(RemainAmtCle);
                Cle.Reset();
                Cle.SetRange("Customer No.", "Sales Invoice Header"."Sell-to Customer No.");
                Cle.SetRange("Document No.", "Sales Invoice Header"."No.");
                Cle.SetRange("Posting Date", "Sales Invoice Header"."Posting Date");
                if Cle.FindFirst() then begin
                    Cle.CalcFields("Remaining Amount");
                    RemainAmtCle := Cle."Remaining Amount";
                end;
                //Invoice Over due text
                if ("Due Date" < WorkDate) And (RemainAmtCle <> 0) then begin
                    InvoiceOverDueText1 := InvoiceOverDuelbl1;
                    InvoiceOverDueText2 := '';
                end
                else begin
                    InvoiceOverDueText1 := InvoiceOverDuelbl2;
                    InvoiceOverDueText2 := '';
                end;
            end;
    ...
    var
        InteractionLogEntryDocumentType: Enum "Interaction Log Entry Document Type";
    begin
        if LogInteraction and not IsReportInPreviewMode then
            if "Sales Invoice Header".FindSet then
                repeat
                    if "Sales Invoice Header"."Bill-to Contact No." <> '' then
                        SegManagement.LogDocument(InteractionLogEntryDocumentType::"Sales Inv.".AsInteger(), "Sales Invoice Header"."No.", 0, 0, DATABASE::Contact, "Sales Invoice Header"."Bill-to Contact No.", "Sales Invoice Header"."Salesperson Code", "Sales Invoice Header"."Campaign No.", "Sales Invoice Header"."Posting Description", '')
                    else
                        SegManagement.LogDocument(InteractionLogEntryDocumentType::"Sales Inv.".AsInteger(), "Sales Invoice Header"."No.", 0, 0, DATABASE::Customer, "Sales Invoice Header"."Bill-to Customer No.", "Sales Invoice Header"."Salesperson Code", "Sales Invoice Header"."Campaign No.", "Sales Invoice Header"."Posting Description", '');
    ...
    var
        ...
        TotalSubTotal_G: Decimal;
        TotalAmount_G: Decimal;
        TotalAmountInclVAT_G: Decimal;
        TotalAmountVAT_G: Decimal;
        TotalInvDiscAmount_G: Decimal;
        RemainAmtCle: Decimal;
        ...
        SalesInvLineCarr: Record "Sales Invoice Line";
        CarriageAmount: Decimal;
        Carriage: Boolean;
        ItemRec: Record Item;
        InvoiceOverDueText1: Text[255];
        InvoiceOverDueText2: Text[255];
        InvoiceOverDuelbl1: Label 'Please find attached a copy of your New invoice. As you can see this invoice is now overdue and requires immediate payment.';
        InvoiceOverDuelbl2: Label 'Please find attached a copy of your New invoice.';
    ...
    begin
        ...
        if LineFeeNoteOnReportHist.FindSet then begin
            ...
            LineFeeNoteOnReportHist.SetRange("Language Code", Language_G.GetUserLanguageCode);
